...
Write-DebugLog "=== CORE MODULE V36.0 (ROOT SOURCES) ===" "INIT"
...
# --- GUI SETUP ---
$Form = New-Object System.Windows.Forms.Form; $Form.Text = "CAI DAT WINDOWS (CORE V36.0)"; $Form.Size = "850, 800"; $Form.StartPosition = "CenterScreen"; $Form.BackColor = [System.Drawing.Color]::FromArgb(30, 30, 30); $Form.ForeColor = "White"; $Form.FormBorderStyle = "FixedSingle"; $Form.MaximizeBox = $false
...
function Start-Boot-Install {
    ...
    try {
        ...
        # --- LOGIC MOI (V36): COPY VAO FOLDER CHUAN \SOURCES DE AUTO DETECT ---
        if ($Global:SourceDrive) {
            $DestDir = "$($Global:SourceDrive):\sources"
        } else {
            # TAT FORMAT
            if ($Content.Contains("<WillWipeDisk>true</WillWipeDisk>")) {
                ...
                [System.Windows.Forms.MessageBox]::Show("CANH BAO: Che do 1 o cung -> Chuyen sang GHI DE (Khong Format).", "Safety")
            }
