<#
  WININSTALL CORE V23.6 (TRINITY FALLBACK & UUID ENGINE)
  Author: Phat Tan PC

  UPDATE V23.6:
  1. Trinity Fallback: Tự động chuyển đổi giữa [Storage Cmdlets] -> [WMI] -> [CMD Native].
  2. Không bao giờ chết trên WinPE cũ (Win 7/8/10 Lite) do thiếu thư viện.
  3. Dùng Native 'mountvol' để ép UUID thành ký tự đĩa ẩn (W:, S:), mượt và an toàn 100%.
  4. Săn Boot (Boot Hunter V3) cân cả EFI định dạng NTFS và nhận diện chuẩn xác.
#>
...
$Form.Text = "WININSTALL CORE V23.6 (TRINITY UUID ENGINE)"
...
$LblTitle.Text = "🚀 WININSTALL V23.6 (TRINITY FALLBACK)"
...
# ==========================================
#   MODE 1: SMART DEPLOY ENGINE (UUID TRINITY)
# ==========================================
$BtnAutoRun.Add_Click({
    ...
    # CƠ CHẾ CẤM TỰ SÁT (Check bằng WMI cho an toàn mọi bản Win)
    ...
    if (-not $Global:IsWinPE -and $OsUUID -and ($OsUUID -eq $TargetSerial)) {
        [System.Windows.Forms.MessageBox]::Show("LỖI CHÍ MẠNG: Đang chạy tool trên Windows sống (Ổ gốc). Đéo được Format tự sát!`n`nVui lòng boot WinPE.", "BẢO VỆ TỰ SÁT", 0, 16)
        return
    }
    ...
    $Pipeline.Commands.AddScript({
        ...
        # Hàm Mount UUID Native bấp chấp WinPE cùi
        ...
        function Smart-Boot-Hunter ($TargetUUID) {
            ...
            if ($HasStorage) {
                try {
                    ...
                    # Quét EFI (Bất kể NTFS hay FAT32)
                    $Efi = Get-Partition -DiskNumber $DiskNum | ? { $_.GptType -eq "{c12a7328-f81f-11d2-ba4b-00a0c93ec93b}" } | Select -First 1
                    if ($Efi) { Write-GuiLog "-> [Lớp 1] Thấy EFI Partition."; return (Get-Volume -Partition $Efi).UniqueId }
                    # Quét Active MBR
                    $Act = Get-Partition -DiskNumber $DiskNum | ? { $_.IsActive -and $_.Size -lt 2GB } | Select -First 1
                    if ($Act) { Write-GuiLog "-> [Lớp 1] Thấy Active Partition."; return (Get-Volume -Partition $Act).UniqueId }
                } catch { Write-GuiLog "-> [Lớp 1] Fail, chuyển Lớp 2..." }
            }

            # Lớp 2: WMI Thần thánh (Nhận dạng cờ System/EFI hoặc Format)
            ...
            if ($BootVol) { 
                Write-GuiLog "-> [Lớp 2] Bắt được Boot Volume: $($BootVol.Label) ($($BootVol.FileSystem))"
                return $BootVol.DeviceID 
            }
            return $null
        }
        ...
        Write-GuiLog "-> Format phân vùng bằng Native CMD (NTFS)..."
        cmd /c "echo Y | format $TargetDrive /fs:ntfs /q /v:Windows >nul 2>&1"

        # 2. XẢ NÉN (API -> DISM)
        Write-GuiLog "=> [2/5] Bắn Image (Index $Idx) thẳng vào Target..."
        $WimSuccess = $false
        if (Get-Command Expand-WindowsImage -ErrorAction SilentlyContinue) {
            Write-GuiLog "-> Đang dùng API Expand-WindowsImage..."
            try { 
                Expand-WindowsImage -ImagePath $Source -Index $Idx -ApplyPath "$TargetDrive\" -ErrorAction Stop | Out-Null
                $WimSuccess = $true 
            } catch { Write-GuiLog "-> API Lỗi: $_. Đá sang DISM..." }
        }
        if (-not $WimSuccess) {
            Write-GuiLog "-> Đang xả nén bằng DISM..."
            cmd /c "dism /Apply-Image /ImageFile:`"$Source`" /Index:$Idx /ApplyDir:$TargetDrive\ 2>&1" | Out-Null
        }
        ...
        # 4. UNATTEND
        Write-GuiLog "=> [4/5] Ghi Unattend OOBE..."
        ...
        # 5. BOOT HUNTER V3
        Write-GuiLog "=> [5/5] Săn Boot và ghi BCD..."
        ...
        Write-GuiLog "======================================================"
        Write-GuiLog "🎉 HOÀN TẤT TRỌN VẸN TRINITY DEPLOY!"
        ...
        $Sync.Btn.Invoke([action]{ 
            ...
            [System.Windows.Forms.MessageBox]::Show("Quá trình Direct Deploy (Trinity) hoàn tất thành công 100%!", "PHÁT TẤN PC VIP")
        })
    }) | Out-Null
...
$BtnManRun.Add_Click({
    ...
    if ([System.Windows.Forms.MessageBox]::Show("Chắc chắn ghi đè Manual?", "Confirm", "YesNo") -eq "Yes") {
        ...
        # Hàm Mount Native nhét tạm cho Manual
        ...
        if ($ChkFmt.Checked) { 
            ...
            cmd /c "echo Y | format $T_Drv /fs:ntfs /q /v:Windows >nul 2>&1"
        }
