...
Write-DebugLog "=== TOOL START V18.0 (FINAL KEY FIX) ==="
...
# --- HÀM MOUNT HYBRID (BẤT TỬ TRÊN MÁY ẢO) ---
...
function Generate-XML {
    ...
    try { 
        ...
        # --- FIX LỖI KEY: DÙNG REGEX XÓA SẠCH CỤM THẺ NẾU TRỐNG ---
        if ([string]::IsNullOrWhiteSpace($Key)) { 
            # Xóa cả thẻ <ProductKey>...</ProductKey>
        ...
        [System.Windows.Forms.MessageBox]::Show("DA TAO XML! (Da Fix loi Key trong)", "Phat Tan PC")
...
function Start-Install ($Mode) {
    ...
    elseif ($Mode -eq "BootTmp") {
        ...
        $DestDir = "C:\WinInstall_Temp"; New-Item -ItemType Directory -Path $DestDir -Force | Out-Null
        Copy-Item $SrcWim "$DestDir\boot.wim" -Force; Copy-Item $SrcSdi "$DestDir\boot.sdi" -Force
        Move-Item "$TempDir\boot.wim" "$SysDrive\WinInstall_Boot.wim" -Force; Move-Item "$TempDir\boot.sdi" "$SysDrive\boot.sdi" -Force
        ...
        } else { [System.Windows.Forms.MessageBox]::Show("Loi copy file!", "Loi") }
    }
