<#
    ...
    Version: 9.5 (WinFsp Fusion + C# Turbo IO + Attribute Fixer)
...
# =========================================================================================
# LÕI C# (EXCEPTION HANDLER + TURBO FILE I/O + ATTRIBUTE FIXER)
# =========================================================================================
...
try {
    ...
    public class PhatTanCore {
        ...
        // HÀM MỚI: TẨY THUỘC TÍNH READ-ONLY CHO FILE WIM ĐỂ DISM/WIMLIB KHÔNG BỊ CHẶN
...
"@
    ...
    Write-Host "[+] Lõi PhatTanCore C# (Turbo I/O) đã được nạp thành công." -ForegroundColor Green
} catch {
    Write-Host "[-] Đang chạy môi trường WinPE/Lite (Thiếu C# Compiler). Tự động kích hoạt Fallback CLI/PS..." -ForegroundColor Yellow
}
...
function Log ($Control, $Msg, $Type="INFO") {
    ...
    if ($Control.InvokeRequired) {
        $Control.Invoke([action]{ $Control.AppendText($Line); $Control.ScrollToCaret() })
    } else {
        $Control.AppendText($Line); $Control.ScrollToCaret()
    }
}
...
$Form.Text = "WINDOWS MODDER STUDIO V9.5 (WINFSP + C# TURBO IO)"
...
$GbAct = New-Object System.Windows.Forms.GroupBox; $GbAct.Text="MENU EDIT (V9.5 WINFSP + C# IO)"; $GbAct.Location="20,100"; $GbAct.Size="845,300"; $GbAct.ForeColor="Lime"; $GbAct.Font = $FontText; $GbAct.Enabled=$false; $TabMod.Controls.Add($GbAct)
...
$BtnCln  = Add-Btn "6. FORCE CLEANUP" 560 30 "Red" { Async-Cleanup }
...
$BtnBuild = New-Object System.Windows.Forms.Button; $BtnBuild.Text="TẠO ISO MỚI (REBUILD BẰNG WIMLIB)"; $BtnBuild.Location="20,410"; $BtnBuild.Size="845,60"; $BtnBuild.BackColor="Green"; $BtnBuild.ForeColor="White"; $BtnBuild.Font=$FontBtnLarge; $BtnBuild.Enabled=$false; $TabMod.Controls.Add($BtnBuild)
    ...
    function Init-Tools ($TDir, $SyncObj) {
        ...
        if (!(Test-Path $WimExe)) {
            try {
                ...
                $SyncObj.TxtMod.Invoke([action]{ $SyncObj.TxtMod.AppendText("[$([DateTime]::Now.ToString('HH:mm:ss'))] [ERR] Lỗi giải nén: $($_.Exception.Message)`r`n") })
            }
        }
        ...
        if (-not $WinFspInstalled) {
            try {
                $SyncObj.TxtMod.Invoke([action]{ $SyncObj.TxtMod.AppendText("[$([DateTime]::Now.ToString('HH:mm:ss'))] [SYS] Cài ngầm WinFsp...`r`n") })
...
function Async-AddFolder {
    ...
    $BtnAddF.Enabled = $false
    $Global:SyncHash.AddSrc = $FBD.SelectedPath
    ...
    $Pipeline = $Runspace.CreatePipeline(); $Pipeline.Commands.AddScript({
        ...
        if ($Sync.HasCSharp -and ('PhatTanCore' -as [type])) {
            [PhatTanCore]::CopyDir($Sync.AddSrc, $DstFolder)
        } else {
            Copy-Item -Path $Sync.AddSrc -Destination $DstFolder -Recurse -Force
        }
...
# --- MOUNT FUSION (FIX ATTRIBUTE ĐỂ DISM/WIMLIB KHÔNG BỊ CHẶN) ---
function Async-Mount {
    ...
    $Pipeline.Commands.AddScript({
        ...
        if ($IsoDrive) {
            ...
            LogBg "Đang tẩy thuộc tính Read-Only của mã nguồn..."
            if ($Sync.HasCSharp -and ('PhatTanCore' -as [type])) {
                [PhatTanCore]::RemoveReadOnly($Sync.ExtractDir)
            } else {
                Get-ChildItem -Path $Sync.ExtractDir -Recurse -Force | % { if ($_.IsReadOnly) { $_.IsReadOnly = $false } }
            }
            ...
            if (-not (Test-Path $SrcW)) {
                LogBg "LỖI: Không tìm thấy file install.wim hoặc install.esd trong thư mục sources!" "ERR"
            ...
            if (Test-Path $WimExe) {
                ...
                if ($P1.ExitCode -eq 0) {
                    ...
                    LogBg "WinFsp không phản hồi (Exit code: $($P1.ExitCode)). Chuyển sang DISM WOF..." "WARN"
                }
            }
            ...
            if ($Sync.MountMethod -eq "NONE") {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    LogBg "Lỗi bung WIM bằng DISM (Exit code: $LASTEXITCODE)!" "ERR" 
                    if ($DismOut.Count -gt 3) { $DismOut = $DismOut[-3..-1] }
                    foreach ($line in $DismOut) { if ($line.Trim() -ne "") { LogBg "-> $line" "ERR" } }
                }
            }
            ...
            LogBg "LỖI: Không Mount được file ISO!" "ERR"
        }
...
# (CÁC HÀM CAPTURE, OPTIMIZE, CLEANUP, REBUILD BÊN DƯỚI GIỮ NGUYÊN NHƯ V9.4)
