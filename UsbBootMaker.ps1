...
# --- CẤU HÌNH ---
# Link JSON gốc (Link Raw de tool doc duoc du lieu)
...
function Load-Kits {
    ...
    try {
        ...
        # Tai JSON tu URL
        $Json = Invoke-RestMethod -Uri $Global:JsonUrl -ErrorAction Stop
        $Global:KitData = $Json
        
        foreach ($Item in $Json) { $CbKit.Items.Add($Item.Name) }
        
        if ($CbKit.Items.Count -gt 0) { 
            $CbKit.SelectedIndex = 0 
            Log-Msg "Da tai thanh cong ($($CbKit.Items.Count) phien ban)."
        }
    } catch {
        Log-Msg "LOI KET NOI GITHUB: $($_.Exception.Message)"
        Log-Msg "Kiem tra lai mang hoac URL JSON."
        $CbKit.Items.Add("Demo Mode (Offline)")
        $CbKit.SelectedIndex = 0
    }
}
...
$Form.Add_Load({ Load-UsbList; Load-Kits; Log-Msg "Ready. Online Mode." })
