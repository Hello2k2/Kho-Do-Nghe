...
[System.IO.File]::WriteAllText($Global:DebugFile, "--- SẴN SÀNG ---`r`n")

# --- 2. HÀM UI & HELPER (Chạy trên luồng chính) ---
function Force-Disk-Refresh-UI {
    try { "rescan" | Out-File "$env:TEMP\dp_rescan.txt" -Encoding ASCII -Force; Start-Process diskpart -ArgumentList "/s `"$env:TEMP\dp_rescan.txt`"" -Wait -WindowStyle Hidden } catch {}
}

function Get-Usb-List-String {
    $L = @(); Force-Disk-Refresh-UI; Start-Sleep 1
...
function Load-Themes-Online {
    $List = @("Mặc định (Ventoy)")
    try { $Global:ThemeData = Invoke-RestMethod -Uri $Global:ThemeConfigUrl -TimeoutSec 3 -EA Stop; foreach ($item in $Global:ThemeData) { if ($item.type -eq "GRUB" -and $item.link) { $List += $item.name } } } catch { }
    return $List
}

# --- 3. LÕI XỬ LÝ ĐA LUỒNG (ASYNC WORKER) ---
# Toàn bộ logic được gói vào ScriptBlock này để truyền vào Runspace
$Global:CoreLogic = {
    param($Config, $G)
    ...
    function Write-SystemLog ($Msg) {
        try { [System.IO.File]::AppendAllText($Global:DebugFile, "[$(Get-Date -F 'HH:mm:ss')] $Msg`r`n") } catch {}
    }

    function Get-Sha256 ($String) {
        $Sha = [System.Security.Cryptography.SHA256]::Create(); $Bytes = [System.Text.Encoding]::UTF8.GetBytes($String); $Hash = $Sha.ComputeHash($Bytes)
    ...
    function Force-Disk-Refresh {
        Write-SystemLog "Rescan Disk..."
        try { "rescan" | Out-File "$env:TEMP\dp_rescan.txt" -Encoding ASCII -Force; Start-Process diskpart -ArgumentList "/s `"$env:TEMP\dp_rescan.txt`"" -Wait -WindowStyle Hidden; Start-Sleep -Seconds 2 } catch {}
    }

    function Get-DriveLetter-DiskPart ($DiskIndex) {
        try { $DpScript = "$env:TEMP\dp_vol_check.txt"; "select disk $DiskIndex`ndetail disk" | Out-File $DpScript -Encoding ASCII -Force; $Output = & diskpart /s $DpScript; foreach ($Line in $Output) { if ($Line -match "Volume \d+\s+([A-Z])\s+") { return "$($Matches[1]):" } } } catch {}; return $null
    }
    ...
    function Prepare-Ventoy-Core {
        ...
        try {
            ...
            if ($LatestVer -ne $CurrentVer -or !(Test-Path "$ExtractPath\ventoy\Ventoy2Disk.exe")) {
                Write-SystemLog "Đang tải Ventoy Core ($LatestVer)... (Vui lòng đợi)"
        ...
        } catch { Write-SystemLog "Dùng bản Offline do lỗi mạng." }
    ...
    Prepare-Ventoy-Core; if (!$Global:VentoyExe) { Write-SystemLog "Lỗi: Không có Ventoy Core!"; return }
    Force-Disk-Refresh; $DL = Get-DriveLetter-DiskPart $Config.DiskID
    if (!$DL) { Write-SystemLog "Lỗi: Không tìm thấy Drive Letter (Ký tự ổ đĩa)!"; return }

    $FlagMode = if ($Config.Mode -eq "Cập nhật Ventoy (Giữ Data)") { "/U" } else { "/I" }
    ...
    Write-SystemLog "Đang chạy lõi cài đặt Ventoy2Disk trên $DL..."
    ...
    if ($P.ExitCode -eq 0) {
        Write-SystemLog "Cài Core Ventoy OK! Đang cấu hình..."
        $UsbRoot = $null
        ...
        if (Test-Path $UsbRoot) {
            ...
            if ($Config.GetMemtest) {
                try {
                    ...
                    if ($RealIso) {
                        ...
                        Copy-Item $RealIso.FullName "$IsoRescueDir\memtest86+.iso" -Force; Write-SystemLog "Cài Memtest OK!"
                    }
                } catch { Write-SystemLog "Lỗi tải Memtest" }
            }
            if ($Config.GetMas) { try { Invoke-WebRequest $Global:MasUrl -OutFile "$UsbRoot\MAS_AIO.cmd" -UseBasicParsing | Out-Null; Write-SystemLog "Tải tool MAS OK" } catch {} }

            # Theme
            $ThemeConfig = $null
            if ($Config.ThemeName -ne "Mặc định (Ventoy)" -and $Config.ThemeLink -ne "") {
                try {
                    Write-SystemLog "Đang tải Theme $($Config.ThemeName)..."
                    $ThemeFile = "$Global:WorkDir\theme_temp.zip"; Download-File-Robust $Config.ThemeLink $ThemeFile
                    ...
                    if ($ThemeTxt) { $RelPath = $ThemeTxt.FullName.Substring($VentoyDir.Length).Replace("\", "/"); $ThemeConfig = "/ventoy$RelPath"; Write-SystemLog "Cài đặt Theme OK!" }
                } catch { Write-SystemLog "Lỗi Theme" }
            }
            ...
            if ($Config.Password -ne "") { $J.Add("password", @{ "menupwd" = (Get-Sha256 $Config.Password) }); Write-SystemLog "Đặt Pass OK (SHA256)" }
            if ($ThemeConfig) { $J.theme.Add("file", $ThemeConfig) }
            
            $J | ConvertTo-Json -Depth 10 | Out-File "$VentoyDir\ventoy.json" -Encoding UTF8 -Force
            Write-SystemLog "=== TIẾN TRÌNH HOÀN TẤT MỌI THỨ ==="
            Write-SystemLog "FLAG_DONE" # Cờ báo hiệu cho Timer biết đã xong
        }
    } else { Write-SystemLog "Lỗi Ventoy ExitCode = $($P.ExitCode)" }
}

# --- 4. BỘ QUẢN LÝ TIẾN TRÌNH (TIMER & RUNSPACE) ---
$Global:UITimer = New-Object System.Windows.Forms.Timer
$Global:UITimer.Interval = 500 # Đọc log mỗi 0.5s

$Global:UITimer.Add_Tick({
    try {
        if (Test-Path $Global:DebugFile) {
            $Lines = [System.IO.File]::ReadAllLines($Global:DebugFile)
            if ($Lines.Count -gt $Global:LastLogLine) {
                for ($i = $Global:LastLogLine; $i -lt $Lines.Count; $i++) {
                    $Msg = $Lines[$i]
                    
                    if ($Msg -match "FLAG_DONE") {
                        $Global:UITimer.Stop(); $Global:AsyncPS.Dispose()
                        if ($Global:UseWpf) { $Global:ActiveStartBtn.IsEnabled = $true } else { $Global:ActiveStartBtn.Enabled = $true }
                    ...
                    if ($Global:ActiveLogControl -ne $null) {
                        if ($Global:UseWpf) {
                            $Global:ActiveLogControl.Dispatcher.Invoke({
                                $Global:ActiveLogControl.AppendText("$Msg`r`n"); $Global:ActiveLogControl.ScrollToEnd()
                            })
                        } else {
                            $Global:ActiveForm.Invoke([Action]{
                                $Global:ActiveLogControl.AppendText("$Msg`r`n"); $Global:ActiveLogControl.ScrollToCaret()
                            })
                        }
                    }
                }
                $Global:LastLogLine = $Lines.Count
            }
        }
    } catch {}

    # Backup tắt Timer nếu Runspace tự ngỏm
    if ($Global:AsyncHandle.IsCompleted -and ($Global:AsyncPS.HadErrors)) {
        $Global:UITimer.Stop()
        if ($Global:UseWpf) { $Global:ActiveStartBtn.IsEnabled = $true } else { $Global:ActiveStartBtn.Enabled = $true }
        [System.Windows.Forms.MessageBox]::Show("Có lỗi xảy ra trong quá trình chạy nền!", "Lỗi")
    }
})

function Start-AsyncJob ($Config) {
    # Reset UI
    if ($Global:UseWpf) { $Global:ActiveLogControl.Dispatcher.Invoke({$Global:ActiveLogControl.Clear()}) } else { $Global:ActiveLogControl.Clear() }
    [System.IO.File]::WriteAllText($Global:DebugFile, "--- BẮT ĐẦU QUÁ TRÌNH ---`r`n")
    ...
    $Script = [scriptblock]::Create($Global:CoreLogic.ToString())
    $PS.AddScript($Script).AddArgument($Config).AddArgument($GlobalsHash) | Out-Null
    
    # Kích hoạt bất đồng bộ và Timer
    $Global:AsyncHandle = $PS.BeginInvoke()
    $Global:AsyncPS = $PS
        ...
        <TabControl Grid.Row="2" Background="#1E1E23" BorderBrush="#00B4FF" Margin="0,0,0,10">
            ...
            <TabItem Header="WIN 11 HACKS" Foreground="Black">
                <StackPanel Margin="15">
                    <CheckBox Name="ChkBypassCheck" Content="Bypass TPM 2.0 &amp; CPU Check" IsChecked="True" Foreground="White" FontSize="14" Margin="0,10"/>
...
"@
    ...
    $Global:ActiveLogControl = $TxtLog; $Global:ActiveStartBtn = $BtnStart; $Global:ActiveForm = $null
    ...
    $BtnStart.Add_Click({
        ...
        if ($Sel -match "Disk (\d+)") {
            if ([System.Windows.Forms.MessageBox]::Show("Toàn bộ dữ liệu trên USB sẽ bị xóa nếu chọn Cài Mới. Tiếp tục?", "Cảnh báo", "YesNo") -eq "Yes") {
                ...
                $ThemeLink = ""
                if ($CbTheme.SelectedItem -ne "Mặc định (Ventoy)" -and $Global:ThemeData) {
                    $T = $Global:ThemeData | Where-Object { $_.Name -eq $CbTheme.SelectedItem } | Select -First 1
                    if ($T) { $ThemeLink = $T.Link }
                }
                ...
                $Config = @{
                    ...
                    ThemeLink=$ThemeLink; MkDir=[bool]$ChkDir.IsChecked; Secure=[bool]$ChkSec.IsChecked
                    GetMemtest=[bool]$ChkMemtest.IsChecked; GetMas=[bool]$ChkLive.IsChecked
                    BypassCheck=[bool]$ChkBypassCheck.IsChecked; BypassNro=[bool]$ChkBypassNRO.IsChecked
                }
                Start-AsyncJob $Config
            }
        }
    })
...
function Show-WinFormsGUI {
    ...
    $L1 = New-Object System.Windows.Forms.Label; $L1.Text = "USB BOOT MASTER - TRUE ASYNC"; $L1.Font = $FTit; $L1.ForeColor = [System.Drawing.Color]::Cyan; $L1.AutoSize = $true; $L1.Location = "10,10"
    ...
    $l_L = New-Object System.Windows.Forms.Label; $l_L.Text="Label:"; $l_L.Bounds="10,$y,100,20"; $txt_L = New-Object System.Windows.Forms.TextBox; $txt_L.Bounds="120,$y,250,20"; $txt_L.Text="PHATTAN_BOOT"; $T1.Controls.AddRange(@($l_L,$txt_L)); $y+=35
    ...
    $BtnS = New-Object System.Windows.Forms.Button; $BtnS.Text = "THỰC HIỆN TẠO BOOT"; $BtnS.Dock = "Bottom"; $BtnS.Height=50; $BtnS.BackColor=[System.Drawing.Color]::Cyan; $BtnS.ForeColor="Black"; $BtnS.Font=$FTit; $F.Controls.Add($BtnS)

    $Global:ActiveLogControl = $TxtL; $Global:ActiveStartBtn = $BtnS; $Global:ActiveForm = $F
    ...
    $BtnS.Add_Click({
        if ($CbU.SelectedItem -match "Disk (\d+)") {
            if ([System.Windows.Forms.MessageBox]::Show("Tiếp tục cài đặt lên USB?", "Xác nhận", "YesNo") -eq "Yes") {
                ...
                $ThemeLink = ""
                if ($cb_Th.SelectedItem -ne "Mặc định (Ventoy)" -and $Global:ThemeData) {
                    $T = $Global:ThemeData | Where-Object { $_.Name -eq $cb_Th.SelectedItem } | Select -First 1
                    if ($T) { $ThemeLink = $T.Link }
                }
                ...
                $Config = @{
                    ...
                    ThemeLink=$ThemeLink; MkDir=$chk_Dir.Checked; Secure=$chk_Sec.Checked
                    GetMemtest=$chk_Mem.Checked; GetMas=$chk_Mas.Checked
